  ...
  <div>
    <div>
      ...
      >
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-form-item label="收入源">
                <span>{{ props.row.incomeRoot }}</span>
              </el-form-item>
        ...
        <el-table-column prop="incomeDate" label="收入日期" width="180" sortable="true"></el-table-column>
        <el-table-column prop="quota" label="金额" sortable="true"></el-table-column>
        <el-table-column prop="incomeRoot" label="收入源"></el-table-column>
      ...
      <el-dialog title="修改数据" :visible.sync="updateFormVisible">
        <el-form :model="form">
          ...
          <el-form-item
            ...
            label-position="labelPosition"
          >
            <el-input v-model="temp.incomeRoot" autocomplete="off"></el-input>
          </el-form-item>
          <el-form-item
            label="收入类型"
            :label-width="formLabelWidth"
      ...
      <el-dialog title="新增数据" :visible.sync="addFormVisible">
         <el-form :model="form">
          ...
          <el-form-item
            ...
            label-position="labelPosition"
          >
            <el-input v-model="temp1.incomeRoot" autocomplete="off"></el-input>
          </el-form-item>
          <el-form-item
            label="收入类型"
            :label-width="formLabelWidth"
...
export default {
  data() {
    return {
      URL: "http://localhost:8080/ffs/income-info",
  ...
  methods: {
    ...
    getAllInfos(currentPage) {
      ...
      try {
        this.$axios.get("/ffs/income-info/idLimit",{params:{page:currentPage}}).then(result => {
          console.log(result)
          this.tableData = result["data"]["data"];

          this.$axios.get("/ffs/income-info/s/idPage").then(num => {
            this.total = num["data"];
          });
        });
      } catch {
    ...
    showUpdateForm(obj) {
      this.show();
      this.temp = obj;
      console.log(this.temp.incomeDate);
    },
    update() {
      this.$axios.get("/ffs/income-info/update", {params:this.temp}).then(result => {
    ...
    del(id) {
      this.AuthConf()
      console.log(id)
      this.$axios.get("/ffs/income-info/del",{params:{id:id}}).then(result => {
        if (result.data.code == 200) {
          this.$notify({
            title: "成功",
    ...
    addinfo(){         
      ...
      this.$axios({
        method:"post",
        url:"/ffs/income-info/add",
        params:{
        quota:that.temp1.quota,
        incomeDate:that.temp1.incomeDate,
        type:that.temp1.type,
        incomeRoot:that.temp1.incomeRoot,
